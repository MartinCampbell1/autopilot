      ...
      {githubPr ? (
        <div className="rounded-2xl border border-[#ecebe8] bg-[#fbfbf9] p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-[14px] font-semibold text-[#37352f]">
                {githubPr.number ? `PR #${githubPr.number}` : githubPr.head_branch}
              </p>
            </div>
            {githubPr.url ? (
              <Link
                href={githubPr.url}
                target="_blank"
                className="inline-flex h-8 items-center rounded-lg border border-[#d3e5ef] bg-[#eef7fb] px-3 text-[12px] font-medium text-[#2a6690] transition-colors hover:bg-[#e3f2f8]"
              >
                Open PR
              </Link>
            ) : null}
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <SessionMetric label="PR State" value={formatGithubValue(githubPr.state)} detail={githubPr.head_branch} />
            <SessionMetric label="CI" value={formatGithubValue(githubPr.ci_status)} detail={githubPr.checks_url || "No checks URL"} />
            <SessionMetric label="Review" value={formatGithubValue(githubPr.review_status)} detail={githubPr.author || "No author"} />
            <SessionMetric label="Handoff" value={formatGithubValue(githubPr.handoff_status)} detail={formatGithubValue(githubPr.merge_state)} />
          </div>
        </div>
      ) : null}
